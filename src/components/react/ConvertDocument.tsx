	...
	return (
		<div className="flex flex-col gap-4">
			<fieldset className="fieldset w-full">
				...
				<select className="select w-full" defaultValue={language} onChange={(e: ChangeEvent<HTMLSelectElement>) => setLanguage(e.target.value as Languages)}>
				...
				<select className="select w-full" defaultValue={format} onChange={(e: ChangeEvent<HTMLSelectElement>) => setFormat(e.target.value as DocumentConversionOutputFormat)}>
			...
			{resultText && <textarea className="textarea textarea-bordered h-64 w-full font-mono" placeholder="API output will appear here…" value={resultText} readOnly />}
		</div>
	);
}
